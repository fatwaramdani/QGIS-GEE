...
#Filter the collection for the VV product after flood
...
Map.addLayer(before, {'min': -14, 'max': -7}, 'Before',0);

after = collectionAft.median().clip(pakistan);
Map.addLayer(after, {'min': -14, 'max': -7}, 'After',0);

#Reduce noise (radar speckle) with smoothing  
smoothing_radius = 100;
before_filtered = before.focal_mean(smoothing_radius, 'circle', 'meters');
after_filtered = after.focal_mean(smoothing_radius, 'circle', 'meters');

Map.addLayer(before_filtered, {'min':-25, 'max':0}, 'Before_filtered',0);
Map.addLayer(after_filtered, {'min':-25, 'max':0}, 'After_filtered',0);
...
#Include JRC layer on surface water seasonality to mask flood pixels from areas of "permanent" water (where there is water > 10 months of the year)
